...
# 将数据中大于阈值的时间归一化处理
def m_test1():
    df = pd.read_csv("data/12345/login-time-abcde-tmp3.csv")
    ixs = df[df["login_order_time"] >= 200000].index

    for ix in list(ixs):
        df.loc[ix, "login_order_time"] = 200000

    df.to_csv("data/12345/login-time-abcde-tmp4.csv", index=False)
    print("over")


# 数据归一化处理(max-min方式)
def m_test2():
    df = pd.read_csv("data/12345/login-time-abcde-tmp4.csv")
    ...
    df.to_csv("data/12345/login-time-abcde-over.csv", index=False)
    print("over")
