...
def dt_extract_data(data, train_data_list):
    ...
    for i in range(len(data)):
        ...
        # 和当前地址前缀有相同前缀的所有数据
        tmp_data = data.loc[0:i+1]
        curr_tmp_data = tmp_data[tmp_data["address_prefix"] == curr_address_prefix]
        # ----------------------提取(前缀 + 详细地址）相似度----------------------
        ...
        curr_tmp_data_new = tmp_data[tmp_data["address_prefix_new"] == address_prefix_new]
        # 相同地址前缀(截断)下的详细地址
...
if __name__ == "__main__":
    data = pd.read_csv("data/12345-over.csv", encoding="GBK")
    ...
    train_data_list.to_csv("data/12345-train.csv", index=False)
    print("over")