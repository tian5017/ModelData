...
# 使用后一列的数据填充缺失值
def m_test1():
    df = pd.read_csv("data/test/login-time-mj-12345.csv")
    # 使用后一列的数据填充此列缺失值
    df = df.fillna(method="bfill", axis=1)
    df.to_csv("data/test/login-time-mj-12345-tmp1.csv", index=False)
    print("over")
...
def m_test2():
    ...
    df_two = pd.read_csv("data/test/login-time-mj-12345-tmp1.csv")
    return_time = []
    ...
    df.to_csv("data/test/login-time-mj-12345-tmp2.csv", index=False)
    print("over")


# 提取登录到下单时间为-1的数据，然后计算平均值，进行填充
def m_test3():
    df = pd.read_csv("data/test/login-time-mj-12345-tmp2.csv")
    ...
    df.to_csv("data/test/login-time-mj-12345-tmp3.csv", index=False)
